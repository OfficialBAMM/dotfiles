...
async function doFormat(context, outputChannel, document, range) {
  ...
  const extensionConfig = import_coc.workspace.getConfiguration("nginx");
  const nginxfmtIndent = extensionConfig.get("nginxfmt.indent", 4);
  let toolPath = extensionConfig.get("nginxfmt.commandPath", "");
  if (!toolPath) {
    if (import_fs.default.existsSync(import_path.default.join(context.storagePath, "nginx-language-server", "venv", "Scripts", "nginxfmt.exe")) || import_fs.default.existsSync(import_path.default.join(context.storagePath, "nginx-language-server", "venv", "bin", "nginxfmt"))) {
      if (process.platform === "win32") {
        toolPath = import_path.default.join(context.storagePath, "nginx-language-server", "venv", "Scripts", "nginxfmt.exe");
      } else {
        toolPath = import_path.default.join(context.storagePath, "nginx-language-server", "venv", "bin", "nginxfmt");
      }
    } else {
...
`);
  ...
  return new Promise(function(resolve) {
    import_child_process.default.execFile(toolPath, [...args, tmpFile.name], opts, function(err) {
      if (err) {
        tmpFile.removeCallback();
        outputChannel.appendLine(`
==== Error ====
${err.message}`);
        import_coc.window.showErrorMessage("There was an error while running nginxfmt.");
        throw err;
      }
...
var NginxFormattingEditProvider = class {
...
var format_default = NginxFormattingEditProvider;
...
// package.json
var nginxLsVersion = "0.7.1";
var nginxFmtVersion = "1.2.2";

// src/constant.ts
var NGINX_LS_VERSION = nginxLsVersion;
var NGINX_FMT_VERSION = nginxFmtVersion;

// src/installer.ts
var exec = import_util.default.promisify(import_child_process2.default.exec);
async function nginxLsInstall(pythonCommand, context) {
  const pathVenv = import_path2.default.join(context.storagePath, "nginx-language-server", "venv");
  let pathVenvPython = import_path2.default.join(context.storagePath, "nginx-language-server", "venv", "bin", "python");
  if (process.platform === "win32") {
    pathVenvPython = import_path2.default.join(context.storagePath, "nginx-language-server", "venv", "Scripts", "python");
  }
  ...
  statusItem.show();
  const installCmd = `${pythonCommand} -m venv ${pathVenv} && ${pathVenvPython} -m pip install -U pip nginx-language-server==${NGINX_LS_VERSION} nginxfmt==${NGINX_FMT_VERSION}`;
  import_rimraf.default.sync(pathVenv);
  try {
    import_coc2.window.showMessage(`Install nginx-language-server and more tools...`);
    await exec(installCmd);
    statusItem.hide();
    import_coc2.window.showMessage(`nginx-language-server: installed!`);
  } catch (error) {
    statusItem.hide();
    import_coc2.window.showErrorMessage(`nginx-language-server: install failed. | ${error}`);
    throw new Error();
  }
}
...
async function activate(context) {
  ...
  const extensionConfig = import_coc3.workspace.getConfiguration("nginx");
  ...
  if (!nginxLsPath) {
    ...
    if (!nginxLsPath) {
      if (import_fs2.default.existsSync(import_path3.default.join(context.storagePath, "nginx-language-server", "venv", "Scripts", "nginx-language-server.exe")) || import_fs2.default.existsSync(import_path3.default.join(context.storagePath, "nginx-language-server", "venv", "bin", "nginx-language-server"))) {
        if (process.platform === "win32") {
          nginxLsPath = import_path3.default.join(context.storagePath, "nginx-language-server", "venv", "Scripts", "nginx-language-server.exe");
        } else {
          nginxLsPath = import_path3.default.join(context.storagePath, "nginx-language-server", "venv", "bin", "nginx-language-server");
        }
      }
    }
  }
  ...
  if (!nginxLsPath || !import_fs2.default.existsSync(nginxLsPath)) {
    import_coc3.window.showErrorMessage('Exit because "nginx-language-server" does not exist.');
    return;
  }
  context.subscriptions.push(import_coc3.commands.registerCommand("nginx.installLanguageServer", async () => {
    if (pythonCommand) {
      if (client.serviceState !== import_coc3.ServiceStat.Stopped) {
        await client.stop();
      }
      await installWrapper(pythonCommand, context);
      client.start();
  ...
  const client = new import_coc3.LanguageClient("nginx-language-server", "Nginx Language Server", serverOptions, clientOptions);
  subscriptions.push(import_coc3.services.registLanguageClient(client));
  const {document} = await import_coc3.workspace.getCurrentState();
  if (document.languageId === "nginx") {
    try {
      await import_coc3.workspace.nvim.command("setlocal iskeyword+=$");
    } catch {
    }
  }
  const formatterOutputChannel = import_coc3.window.createOutputChannel("nginx-format");
  const editProvider = new format_default(context, formatterOutputChannel);
  const priority = 1;
  const languageSelector = [{language: "nginx", scheme: "file"}];
  function registerFormatter() {
    disposeHandlers();
    formatterHandler = import_coc3.languages.registerDocumentFormatProvider(languageSelector, editProvider, priority);
  }
  registerFormatter();
  context.subscriptions.push(import_coc3.commands.registerCommand("nginx.format", async () => {
    const doc = await import_coc3.workspace.document;
    const code = await doFormat(context, formatterOutputChannel, doc.textDocument, void 0);
...
function whichNginxLsCommand() {
  try {
    return import_which.default.sync("nginx-language-server");
  } catch (e) {
    return "";
  }
}
