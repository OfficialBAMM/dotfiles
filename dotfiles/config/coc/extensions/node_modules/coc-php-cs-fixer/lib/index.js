...
// src/actions/pcfFix.ts
var import_coc = require("coc.nvim");
function activate(context) {
  const documentSelector = [{ language: "php", scheme: "file" }];
  if (import_coc.workspace.getConfiguration("php-cs-fixer").get("enableActionProvider", true)) {
    context.subscriptions.push(import_coc.languages.registerCodeActionProvider(documentSelector, new PcfFixCodeActionProvider(), "php-cs-fixer"));
  }
}
var PcfFixCodeActionProvider = class {
...
// src/actions/pintFix.ts
var import_coc2 = require("coc.nvim");
function activate2(context) {
  const documentSelector = [{ language: "php", scheme: "file" }];
  if (import_coc2.workspace.getConfiguration("php-cs-fixer").get("enableActionProvider", true)) {
    context.subscriptions.push(import_coc2.languages.registerCodeActionProvider(documentSelector, new PintFixCodeActionProvider(), "php-cs-fixer"));
  }
}
var PintFixCodeActionProvider = class {
  async provideCodeActions(document2, range, context) {
    const codeActions = [];
    if (document2.languageId !== "php")
      return;
    const doc = import_coc2.workspace.getDocument(document2.uri);
    if (this.wholeRange(doc, range)) {
...
// src/commands/pcfDownload.ts
var import_coc4 = require("coc.nvim");

// src/downloaders/pcfDownloader.ts
...
function getNodeRequestOptions(request) {
  ...
  if (typeof agent3 === "function") {
    agent3 = agent3(parsedURL);
  }
  if (!headers.has("Connection") && !agent3) {
...
async function download(context, downloadMajorVersion) {
  ...
  const _path = import_path.default.join(context.storagePath, "php-cs-fixer");
  const randomHex = (0, import_crypto.randomBytes)(5).toString("hex");
  const tempFile = import_path.default.join(context.storagePath, `php-cs-fixer-${randomHex}`);
...
function activate3(context) {
  context.subscriptions.push(import_coc4.commands.registerCommand("php-cs-fixer.download", runDownloadCommand(context)));
}
function runDownloadCommand(context) {
  return async () => {
    const downloadMajorVersion = import_coc4.workspace.getConfiguration("php-cs-fixer").get("downloadMajorVersion", 3);
...
async function downloadWrapper(context, downloadMajorVersion) {
  ...
  const ret = await import_coc4.window.showPrompt(msg);
  if (ret) {
    try {
      await download(context, downloadMajorVersion);
      import_coc4.commands.executeCommand("editor.action.restart");
    } catch (e) {
      console.error(e);
      msg = "Download php-cs-fixer failed, you can get it from https://github.com/FriendsOfPHP/PHP-CS-Fixer";
      import_coc4.window.showErrorMessage(msg);
      return;
    }
  } else {
    return;
...
function fullDocumentRange(document2) {
  ...
  const doc = import_coc5.workspace.getDocument(document2.uri);
  return import_coc5.Range.create({ character: 0, line: 0 }, { character: doc.getline(lastLineId).length, line: lastLineId });
}
function getPcfPath(context) {
  let toolPath = import_coc5.workspace.getConfiguration("php-cs-fixer").get("toolPath", "");
  if (!toolPath) {
    if (import_fs2.default.existsSync(import_path2.default.join(import_coc5.workspace.root, "vendor", "bin", "php-cs-fixer"))) {
      toolPath = import_path2.default.join(import_coc5.workspace.root, "vendor", "bin", "php-cs-fixer");
    } else if (import_fs2.default.existsSync(import_path2.default.join(context.storagePath, "php-cs-fixer"))) {
      toolPath = import_path2.default.join(context.storagePath, "php-cs-fixer");
    }
  }
  return toolPath;
}
function getPintPath(context) {
  let toolPath = import_coc5.workspace.getConfiguration("php-cs-fixer").get("pint.toolPath", "");
  if (!toolPath) {
    if (import_fs2.default.existsSync(import_path2.default.join(import_coc5.workspace.root, "vendor", "bin", "pint"))) {
      toolPath = import_path2.default.join(import_coc5.workspace.root, "vendor", "bin", "pint");
    } else if (import_fs2.default.existsSync(import_path2.default.join(context.storagePath, "pint"))) {
      toolPath = import_path2.default.join(context.storagePath, "pint");
    }
  }
  return toolPath;
}
function resolveConfigPath(configPath, cwd) {
  if (!import_path2.default.isAbsolute(configPath)) {
    let currentPath = cwd;
    const triedPaths = [currentPath];
    while (!import_fs2.default.existsSync(currentPath + import_path2.default.sep + configPath)) {
      const lastPath = currentPath;
      currentPath = import_path2.default.dirname(currentPath);
      if (lastPath == currentPath) {
        import_coc5.window.showErrorMessage(`Unable to find ${configPath} file in ${triedPaths.join(", ")}`);
        return "";
      } else {
        triedPaths.push(currentPath);
      }
    }
    configPath = currentPath + import_path2.default.sep + configPath;
  }
  return configPath;
}
function isExistsFixerConfigFileFromProjectRoot() {
  return import_fs2.default.existsSync(import_path2.default.join(import_coc5.workspace.root, ".php-cs-fixer.php")) || import_fs2.default.existsSync(import_path2.default.join(import_coc5.workspace.root, ".php-cs-fixer.dist.php"));
}
function isExistsPintConfigFileFromProjectRoot() {
  return import_fs2.default.existsSync(import_path2.default.join(import_coc5.workspace.root, "pint.json"));
}

// src/commands/pcfDryRunDiff.ts
var terminal;
function activate4(context) {
  context.subscriptions.push(import_coc6.commands.registerCommand("php-cs-fixer.dryRunDiff", pcfDryRunDiffCommand(context)));
}
async function runPcfDryRunDiff(context, filePath) {
  const pcfBin = getPcfPath(context);
  if (pcfBin) {
    if (terminal) {
      if (terminal.bufnr) {
        await import_coc6.workspace.nvim.command(`bd! ${terminal.bufnr}`);
      }
      terminal.dispose();
      terminal = void 0;
    }
    const extensionConfig = import_coc6.workspace.getConfiguration("php-cs-fixer");
    ...
    const enableIgnoreEnv = extensionConfig.get("enableIgnoreEnv", false);
    const existsFixerConfigFile = isExistsFixerConfigFileFromProjectRoot();
    const cwd = import_coc6.workspace.root;
    let env = void 0;
    if (enableIgnoreEnv) {
      env = {
        ...process.env,
    ...
    terminal = await import_coc6.window.createTerminal({ name: "pcf-dry-run-diff", cwd, env });
    const args = [];
    ...
    const enableSplitRight = import_coc6.workspace.getConfiguration("php-cs-fixer").get("terminal.enableSplitRight", false);
    if (enableSplitRight)
      terminal.hide();
    await import_coc6.workspace.nvim.command("stopinsert");
    if (enableSplitRight) {
      await import_coc6.workspace.nvim.command(`vert bel sb ${terminal.bufnr}`);
      await import_coc6.workspace.nvim.command(`wincmd p`);
    }
  } else {
    return import_coc6.window.showErrorMessage("php-cs-fixer not found!");
  }
}
function pcfDryRunDiffCommand(context) {
  return async () => {
    const { document: document2 } = await import_coc6.workspace.getCurrentState();
    const filePath = import_coc6.Uri.parse(document2.uri).fsPath;
    if (document2.languageId !== "php") {
      return import_coc6.window.showErrorMessage("This file is not a PHP file!");
    }
    runPcfDryRunDiff(context, filePath);
  };
}

// src/commands/pcfFix.ts
var import_coc8 = require("coc.nvim");

// src/engines/pcfEngine.ts
...
async function doFormat(context, outputChannel, document2, range) {
  if (document2.languageId !== "php") {
    import_coc7.window.showErrorMessage(`php-cs-fixer.fix cannot run, not a php file`);
    return;
  }
  const filepath = import_coc7.Uri.parse(document2.uri).fsPath;
  const filename = import_path3.default.basename(filepath);
  if (filename === ".php-cs-fixer.php" || filename === ".php-cs-fixer.dist.php") {
    import_coc7.window.showWarningMessage(`php-cs-fixer config file is excluded from the formatting process.`);
    return;
  }
  const extensionConfig = import_coc7.workspace.getConfiguration("php-cs-fixer");
  const isUseCache = extensionConfig.get("useCache", false);
  const isAllowRisky = extensionConfig.get("allowRisky", true);
  const extensionFixerConfig = extensionConfig.get("config", "");
  const fixerRules = extensionConfig.get("rules", "@PSR12");
  ...
  if (!toolPath) {
    import_coc7.window.showErrorMessage(`Unable to find the php-cs-fixer tool.`);
    return;
  }
...
`);
  return new Promise(function(resolve) {
    import_child_process.default.execFile("php", [...args, tmpFile.name], opts, function(err, stdout, stderr) {
      if (err) {
...
function activate5(context, outputChannel) {
  context.subscriptions.push(import_coc8.commands.registerCommand("php-cs-fixer.fix", runFixCommand(context, outputChannel)));
}
function runFixCommand(context, outputChannel) {
  return async () => {
    const doc = await import_coc8.workspace.document;
    const code = await doFormat(context, outputChannel, doc.textDocument, void 0);
    if (!code)
      return;
    const edits = [import_coc8.TextEdit.replace(fullDocumentRange(doc.textDocument), code)];
    if (edits) {
...
// src/commands/pintDownload.ts
var import_coc10 = require("coc.nvim");

// src/downloaders/pintDownloader.ts
var import_coc9 = require("coc.nvim");
var import_crypto2 = require("crypto");
var import_fs4 = require("fs");
var import_https_proxy_agent2 = __toESM(require_dist());
var import_path4 = __toESM(require("path"));
var import_stream3 = __toESM(require("stream"));
var import_util2 = __toESM(require("util"));
var pipeline2 = import_util2.default.promisify(import_stream3.default.pipeline);
var agent2 = process.env.https_proxy ? new import_https_proxy_agent2.HttpsProxyAgent(process.env.https_proxy) : null;
async function download2(context) {
  const statusItem = import_coc9.window.createStatusBarItem(0, { progress: true });
  statusItem.text = `Downloading pint`;
  statusItem.show();
  const downloadUrl = "https://github.com/laravel/pint/releases/latest/download/pint.phar";
  const resp = await lib_default(downloadUrl, { agent: agent2 });
  if (!resp.ok) {
    statusItem.hide();
    throw new Error("Download failed");
  }
  ...
  const _path = import_path4.default.join(context.storagePath, "pint");
  const randomHex = (0, import_crypto2.randomBytes)(5).toString("hex");
  const tempFile = import_path4.default.join(context.storagePath, `pint-${randomHex}`);
  const destFileStream = (0, import_fs4.createWriteStream)(tempFile, { mode: 493 });
  await pipeline2(resp.body, destFileStream);
  await new Promise((resolve) => {
    destFileStream.on("close", resolve);
    destFileStream.destroy();
    setTimeout(resolve, 1e3);
  });
  await import_fs4.promises.unlink(_path).catch((err) => {
    if (err.code !== "ENOENT")
      throw err;
  });
  await import_fs4.promises.rename(tempFile, _path);
  statusItem.hide();
}

// src/commands/pintDownload.ts
function activate6(context) {
  context.subscriptions.push(import_coc10.commands.registerCommand("php-cs-fixer.pintDownload", runDownloadCommand2(context)));
}
function runDownloadCommand2(context) {
  return async () => {
    await downloadWrapper2(context);
  };
}
async function downloadWrapper2(context) {
  let msg = 'Do you want to download "pint"?';
  const ret = await import_coc10.window.showPrompt(msg);
  if (ret) {
    try {
      await download2(context);
      import_coc10.commands.executeCommand("editor.action.restart");
    } catch (e) {
      console.error(e);
      msg = "Download pint failed, you can get it from https://github.com/laravel/pint/releases";
      import_coc10.window.showErrorMessage(msg);
      return;
    }
  } else {
    return;
...
// src/commands/pintFix.ts
var import_coc12 = require("coc.nvim");

// src/engines/pintEngine.ts
var import_coc11 = require("coc.nvim");
var import_child_process2 = __toESM(require("child_process"));
var import_fs5 = __toESM(require("fs"));
var import_tmp2 = __toESM(require_tmp());
async function doFormat2(context, outputChannel, document2, range) {
  if (document2.languageId !== "php") {
    import_coc11.window.showErrorMessage(`php-cs-fixer.pint cannot run, not a php file`);
    return;
  }
  const extensionConfig = import_coc11.workspace.getConfiguration("php-cs-fixer");
  ...
  if (!toolPath) {
    import_coc11.window.showErrorMessage(`Unable to find the pint tool.`);
    return;
  }
  const text = document2.getText(range);
  const args = [];
  const cwd = import_coc11.Uri.file(import_coc11.workspace.root).fsPath;
  ...
  const tmpFile = import_tmp2.default.fileSync();
...
`);
  return new Promise(function(resolve) {
    import_child_process2.default.execFile("php", [...args, tmpFile.name], opts, function(err, stdout, stderr) {
      if (err) {
        tmpFile.removeCallback();
        if (err.code === "ENOENT") {
          import_coc11.window.showErrorMessage("Unable to find the pint tool.");
...
`);
      ...
      const text2 = import_fs5.default.readFileSync(tmpFile.name, "utf-8");
      tmpFile.removeCallback();
      resolve(text2);
    });
  });
...
// src/commands/pintFix.ts
function activate7(context, outputChannel) {
  context.subscriptions.push(import_coc12.commands.registerCommand("php-cs-fixer.pintFix", runFixCommand2(context, outputChannel)));
}
function runFixCommand2(context, outputChannel) {
  return async () => {
    const doc = await import_coc12.workspace.document;
    const code = await doFormat2(context, outputChannel, doc.textDocument, void 0);
    if (!code)
      return;
    const edits = [import_coc12.TextEdit.replace(fullDocumentRange(doc.textDocument), code)];
    if (edits) {
      await doc.applyEdits(edits);
    }
  };
...
// src/commands/pintTest.ts
var import_coc13 = require("coc.nvim");
var terminal2;
function activate8(context) {
  context.subscriptions.push(import_coc13.commands.registerCommand("php-cs-fixer.pintTest", pintTestCommand(context)));
}
async function runPintTest(context, filePath) {
  const pintBin = getPintPath(context);
  if (pintBin) {
    if (terminal2) {
      if (terminal2.bufnr) {
        await import_coc13.workspace.nvim.command(`bd! ${terminal2.bufnr}`);
      }
      terminal2.dispose();
      terminal2 = void 0;
    }
    const cwd = import_coc13.workspace.root;
    terminal2 = await import_coc13.window.createTerminal({ name: "pint-test", cwd });
    const args = [];
    const extensionConfig = import_coc13.workspace.getConfiguration("php-cs-fixer");
    const extensionPintConfig = extensionConfig.get("pint.config", "");
    const preset = extensionConfig.get("pint.preset", "laravel");
    const existsPintConfigFile = isExistsPintConfigFileFromProjectRoot();
    if (extensionPintConfig) {
    ...
    args.push("--test");
    args.push(`${filePath}`);
    terminal2.sendText(`${pintBin} ${args.join(" ")}`);
    const enableSplitRight = import_coc13.workspace.getConfiguration("php-cs-fixer").get("terminal.enableSplitRight", false);
    if (enableSplitRight)
      terminal2.hide();
    await import_coc13.workspace.nvim.command("stopinsert");
    if (enableSplitRight) {
      await import_coc13.workspace.nvim.command(`vert bel sb ${terminal2.bufnr}`);
      await import_coc13.workspace.nvim.command(`wincmd p`);
    }
  } else {
    return import_coc13.window.showErrorMessage("pint not found!");
  }
}
function pintTestCommand(context) {
  return async () => {
    const { document: document2 } = await import_coc13.workspace.getCurrentState();
    const filePath = import_coc13.Uri.parse(document2.uri).fsPath;
    if (document2.languageId !== "php") {
      return import_coc13.window.showErrorMessage("This file is not a PHP file!");
    }
...
function activate9(context, outputChannel) {
  context.subscriptions.push(import_coc14.commands.registerCommand("php-cs-fixer.showOutput", () => {
    outputChannel.show();
  }));
}

// src/documentFormats/pcfFix.ts
var import_coc15 = require("coc.nvim");
function activate10(context, outputChannel) {
  if (import_coc15.workspace.getConfiguration("php-cs-fixer").get("enableFormatProvider", true)) {
    const languageSelector = [{ language: "php", scheme: "file" }];
    const priority = 1;
    context.subscriptions.push(import_coc15.languages.registerDocumentFormatProvider(languageSelector, new PcfFixFormattingEditProvider(context, outputChannel), priority));
  }
}
var PcfFixFormattingEditProvider = class {
  ...
  async _provideEdits(document2, range) {
    ...
    return [import_coc15.TextEdit.replace(range, code)];
  }
};

// src/documentFormats/pintFix.ts
var import_coc16 = require("coc.nvim");
function activate11(context, outputChannel) {
  if (import_coc16.workspace.getConfiguration("php-cs-fixer").get("enableFormatProvider", true)) {
    const languageSelector = [{ language: "php", scheme: "file" }];
    const priority = 1;
    context.subscriptions.push(import_coc16.languages.registerDocumentFormatProvider(languageSelector, new PintFixFormattingEditProvider(context, outputChannel), priority));
  }
}
var PintFixFormattingEditProvider = class {
  constructor(context, outputChannel) {
    this._context = context;
    this._outputChannel = outputChannel;
  }
  ...
  async _provideEdits(document2, range) {
    const code = await doFormat2(this._context, this._outputChannel, document2, range);
    if (!code)
      return [];
    if (!range) {
      range = fullDocumentRange(document2);
    }
    return [import_coc16.TextEdit.replace(range, code)];
  }
};

// src/statusBar.ts
var import_coc17 = require("coc.nvim");
async function activate12(context) {
  const statusBar = import_coc17.window.createStatusBarItem(99);
  updateStatusBar();
  import_coc17.events.on("BufEnter", async () => {
    updateStatusBar();
  }, null, context.subscriptions);
  async function updateStatusBar() {
    const { document: document2 } = await import_coc17.workspace.getCurrentState();
    if (!import_coc17.workspace.getConfiguration("php-cs-fixer").get("enable")) {
      statusBar.hide();
    } else if (["php"].includes(document2.languageId)) {
      const activateTool = import_coc17.workspace.getConfiguration("php-cs-fixer").get("activateTool", "php-cs-fixer");
...
// src/index.ts
async function activate13(context) {
  if (!import_coc18.workspace.getConfiguration("php-cs-fixer").get("enable", true))
    return;
  const extensionStoragePath = context.storagePath;
  if (!import_fs6.default.existsSync(extensionStoragePath)) {
    import_fs6.default.mkdirSync(extensionStoragePath);
  }
  const outputChannel = import_coc18.window.createOutputChannel("php-cs-fixer");
  activate9(context, outputChannel);
  activate3(context);
  activate6(context);
  const activateTool = import_coc18.workspace.getConfiguration("php-cs-fixer").get("activateTool", "php-cs-fixer");
  let toolPath;
  if (activateTool === "php-cs-fixer") {
    toolPath = getPcfPath(context);
    if (import_coc18.workspace.getConfiguration("php-cs-fixer").get("downloadCheckOnStartup", true)) {
      if (!toolPath) {
        import_coc18.commands.executeCommand("php-cs-fixer.download");
      }
    }
  } else if (activateTool === "pint") {
    ...
    if (import_coc18.workspace.getConfiguration("php-cs-fixer").get("downloadCheckOnStartup", true)) {
      if (!toolPath) {
        import_coc18.commands.executeCommand("php-cs-fixer.pintDownload");
      }
    }
  }
  if (!toolPath)
  ...
  if (activateTool === "php-cs-fixer") {
    ...
    activate4(context);
    activate10(context, outputChannel);
    activate(context);
  } else if (activateTool === "pint") {
    activate7(context, outputChannel);
    activate8(context);
    activate11(context, outputChannel);
    activate2(context);
  }
  if (activateTool === "php-cs-fixer" || activateTool === "pint")
    activate12(context);
}
